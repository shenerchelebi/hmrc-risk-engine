...
const ResultsPage = () => {
  ...
  const getRiskColor = (band) => {
    switch (band) {
      case 'LOW': return { bg: 'bg-emerald-100', text: 'text-emerald-800', border: 'border-emerald-200', pulse: 'risk-pulse-low' };
      case 'MODERATE': return { bg: 'bg-amber-100', text: 'text-amber-800', border: 'border-amber-200', pulse: 'risk-pulse-moderate' };
      case 'HIGH': return { bg: 'bg-rose-100', text: 'text-rose-800', border: 'border-rose-200', pulse: 'risk-pulse-high' };
      default: return { bg: 'bg-stone-100', text: 'text-stone-800', border: 'border-stone-200', pulse: '' };
    }
  };
  ...
  if (!assessment) {
    return (
      <div className="min-h-screen bg-stone-50 flex flex-col items-center justify-center p-6">
        ...
        <h1 className="font-serif text-2xl font-bold text-slate-900 mb-2">Assessment Not Found</h1>
        <p className="text-stone-600 mb-6">The assessment you're looking for doesn't exist.</p>
      ...
      <main className="px-6 md:px-12 py-12">
        <div className="max-w-3xl mx-auto">
          ...
          <Card className="card-elevated overflow-hidden mb-8">
            <div className={`h-2 ${riskColors.bg}`}></div>
            <CardContent className="p-8 md:p-12 text-center">
              <p className="text-stone-600 mb-2">Your HMRC Risk Score</p>
              
              <div className={`score-circle inline-flex items-center justify-center w-40 h-40 rounded-full ${riskColors.bg} ${riskColors.border} border-4 ${riskColors.pulse} mb-6`}>
              ...
              <Badge 
                className={`${riskColors.bg} ${riskColors.text} ${riskColors.border} text-lg px-6 py-2 mb-6`}
          ...
          <Card className="card-elevated mb-8">
            <CardContent className="p-8">
              <h3 className="font-serif text-xl font-semibold text-slate-900 mb-6">Assessment Summary</h3>
              
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div className="flex justify-between py-2 border-b border-stone-100">
                    <span className="text-stone-600">Tax Year</span>
                    <span className="font-medium text-slate-900">{assessment.tax_year}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-stone-100">
                    <span className="text-stone-600">Turnover</span>
                    <span className="font-medium text-slate-900">£{assessment.turnover?.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-stone-100">
                    <span className="text-stone-600">Profit</span>
                    <span className="font-medium text-slate-900">£{assessment.profit?.toLocaleString()}</span>
                  </div>
                </div>
                <div className="space-y-4">
                  <div className="flex justify-between py-2 border-b border-stone-100">
                    <span className="text-stone-600">Risk Indicators</span>
                    <span className="font-medium text-slate-900">{assessment.triggered_rules_count} triggered</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-stone-100">
                    <span className="text-stone-600">Email</span>
                    <span className="font-medium text-slate-900 text-sm">{assessment.email}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-stone-100">
                    <span className="text-stone-600">Report Status</span>
                    <span className={`font-medium ${assessment.payment_status === 'paid' ? 'text-emerald-600' : 'text-amber-600'}`}>
          ...
          {assessment.payment_status === 'paid' ? (
            ...
            <Card className="card-elevated border-l-4 border-l-teal-600">
              <CardContent className="p-8">
                <div className="flex flex-col md:flex-row md:items-center gap-6">
                  ...
                  <div className="text-center md:text-right">
                    <p className="text-3xl font-bold text-slate-900 mb-2">£19.99</p>
                    <p className="text-sm text-stone-500 mb-4">One-time payment</p>
                    <Button
                      onClick={handlePurchase}
                      disabled={checkoutLoading}
                      className="bg-teal-600 hover:bg-teal-500 text-white px-8 py-6 text-lg rounded-lg w-full md:w-auto"
          ...
          {/* Disclaimer */}
          <p className="text-xs text-stone-500 text-center mt-8 max-w-lg mx-auto">
